use crate::core::{
    normalize::{label_for_minutes, normalize_window},
    redaction::redact,
...
async fn fetch_inner() -> Result<ProviderUsage, String> {
    ...
    send(
        &mut writer,
        &json!({"id":1,"method":"initialize","params":{"clientInfo":{"name":"usage","title":"Usage","version":"0.1.0"},"capabilities":{"experimentalApi":true}}}),
    )
    .await?;
    ...
    let response: RateLimitsResponse = serde_json::from_value(value)
        .map_err(|_| "Codex rate-limit response schema changed".to_string())?;
    ...
    if let Some(w) = snapshot.primary {
        windows.push(normalize_window(
            "primary",
            label_for_minutes(w.window_duration_mins, "Primary"),
            w.used_percent,
            w.resets_at,
        ));
    }
    if let Some(w) = snapshot.secondary {
        windows.push(normalize_window(
            "secondary",
            label_for_minutes(w.window_duration_mins, "Secondary"),
            w.used_percent,
            w.resets_at,
        ));
    }
    if windows.is_empty() {
        return Err("Codex returned no subscription usage windows".into());
    }
...
async fn wait_result(
    ...
    timeout(REQUEST_TIMEOUT, async {
        while let Some(line) = reader
            .next_line()
            .await
            .map_err(|_| "Unable to read Codex app-server")?
        {
            ...
            return value
                .get("result")
                .cloned()
                .ok_or_else(|| "Codex response missing result".to_string());
        }
...
mod tests {
    ...
    #[test]
    fn redacts_provider_error_text() {
