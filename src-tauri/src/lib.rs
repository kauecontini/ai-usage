...
use chrono::Utc;
use std::{collections::{HashMap, HashSet}, fs::OpenOptions, io::Write, sync::{Arc, Mutex, RwLock}, time::Duration};
use tauri::{menu::{Menu, MenuItem}, tray::TrayIconBuilder, AppHandle, Manager, PhysicalPosition, State, WebviewWindow};
...
impl SharedState {
    fn new(paths: AppPaths) -> Self {
        ...
        let cached = std::fs::read_to_string(&paths.cache).ok()
...
fn mark_cache_stale(mut snapshot: UsageSnapshot) -> UsageSnapshot {
    for provider in &mut snapshot.providers {
        if !provider.windows.is_empty() { provider.status = ProviderStatus::Stale; }
    }
    snapshot
}
...
fn get_settings(state: State<'_, SharedState>) -> AppSettings {
    state.settings.read().expect("settings lock poisoned").clone()
}

#[tauri::command]
async fn refresh_now(app: AppHandle, state: State<'_, SharedState>) -> Result<UsageSnapshot, String> {
    Ok(refresh_all(&app, state.inner().clone(), true).await)
}

#[tauri::command]
fn save_settings(app: AppHandle, state: State<'_, SharedState>, settings: AppSettings) -> Result<AppSettings, String> {
    let validated = settings.validate();
    apply_runtime_settings(&app, &validated)?;
    settings::save(&state.paths.settings, &validated).map_err(|_| "Unable to save settings".to_string())?;
    *state.settings.write().map_err(|_| "Settings unavailable".to_string())? = validated.clone();
    Ok(validated)
}
...
fn set_surface(window: WebviewWindow, surface: String) -> Result<(), String> {
    ...
    window.set_size(tauri::LogicalSize::new(width, height)).map_err(|_| "Unable to resize Usage".to_string())
}

fn apply_runtime_settings(app: &AppHandle, settings: &AppSettings) -> Result<(), String> {
    if let Some(window) = app.get_webview_window("main") {
        window.set_always_on_top(settings.always_on_top).map_err(|_| "Unable to change always-on-top".to_string())?;
    }
    let manager = app.autolaunch();
    if settings.launch_at_startup {
        manager.enable().map_err(|_| "Unable to enable launch at startup".to_string())?;
    } else {
        manager.disable().map_err(|_| "Unable to disable launch at startup".to_string())?;
    }
    Ok(())
}
...
async fn refresh_all(app: &AppHandle, state: SharedState, force: bool) -> UsageSnapshot {
    ...
    let merged_openai = if let Some(current) = openai {
        ...
        previous
            ...
            .find(|p| p.provider == ProviderId::Openai)
            .cloned()
            .unwrap_or_else(|| crate::core::types::ProviderUsage::empty(ProviderId::Openai, "codex_app_server"))
    };
    ...
    let merged_anthropic = if let Some(current) = anthropic {
        ...
        previous
            ...
            .find(|p| p.provider == ProviderId::Anthropic)
            .cloned()
            .unwrap_or_else(|| crate::core::types::ProviderUsage::empty(ProviderId::Anthropic, "claude_oauth_usage_unofficial"))
    };
...
fn maybe_notify(app: &AppHandle, state: &SharedState, snapshot: &UsageSnapshot) {
    if !state.settings.read().expect("settings lock poisoned").notifications { return; }
    let mut sent = state.notifications.lock().expect("notification lock poisoned");
    for provider in &snapshot.providers {
        if provider.status != ProviderStatus::Fresh { continue; }
        for window in &provider.windows {
            let Some(remaining) = window.remaining_percent else { continue; };
            let threshold = if remaining <= 5.0 { Some(5) } else if remaining <= 10.0 { Some(10) } else if remaining <= 20.0 { Some(20) } else { None };
            let Some(threshold) = threshold else { continue; };
            let key = format!("{:?}:{}:{}", provider.provider, window.id, window.reset_at.unwrap_or_default());
            let thresholds = sent.entry(key).or_default();
            if !thresholds.insert(threshold) { continue; }
            let name = match provider.provider { ProviderId::Openai => "ChatGPT / Codex", ProviderId::Anthropic => "Claude" };
            let _ = app.notification().builder().title("Usage").body(format!("{name}: {}% remaining ({})", remaining.round(), window.label)).show();
        }
    }
}
...
fn setup_tray(app: &tauri::App) -> tauri::Result<()> {
    ...
    let mut builder = TrayIconBuilder::with_id("usage-tray").menu(&menu).show_menu_on_left_click(false).tooltip("Usage");
    ...
    builder.on_tray_icon_event(|tray, event| {
        ...
        "refresh" => { let app = app.clone(); let state = app.state::<SharedState>().inner().clone(); tauri::async_runtime::spawn(async move { refresh_all(&app, state, true).await; }); },
        "logs" => { let state = app.state::<SharedState>(); #[cfg(windows)] { let _ = std::process::Command::new("explorer.exe").arg(&state.paths.logs).spawn(); } },
        "quit" => app.exit(0),
        _ => {}
    }).build(app)?;
    Ok(())
...
fn restore_position(window: &WebviewWindow, settings: &AppSettings) {
    ...
    for monitor in &monitors {
        ...
        let visibly_intersects = x.saturating_add(80) > left
            && x < right
            && y.saturating_add(40) > top
            && y < bottom;
        if visibly_intersects {
            let max_x = right.saturating_sub(width).max(left);
            let max_y = bottom.saturating_sub(height).max(top);
            let _ = window.set_position(PhysicalPosition::new(x.clamp(left, max_x), y.clamp(top, max_y)));
            return;
        }
    }
...
fn start_scheduler(app: AppHandle, state: SharedState) {
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            let minutes = state.settings.read().expect("settings lock poisoned").refresh_interval_minutes;
...
pub fn run() {
    ...
    let initial_settings = state.settings.read().expect("settings lock poisoned").clone();
    ...
    builder.manage(state.clone())
        ...
        .invoke_handler(tauri::generate_handler![get_snapshot, get_settings, refresh_now, save_settings, set_surface])
...
mod tests {
    ...
    fn cache_is_marked_stale() {
        ...
        snapshot.providers[0].windows.push(crate::core::normalize::normalize_window("p", "5 hours", 20.0, None));
        snapshot.providers[0].status = ProviderStatus::Fresh;
        assert_eq!(mark_cache_stale(snapshot).providers[0].status, ProviderStatus::Stale);
    }
}
