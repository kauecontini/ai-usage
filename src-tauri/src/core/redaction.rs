...
pub fn redact(input: &str) -> String {
    ...
    let bearer = BEARER.get_or_init(|| Regex::new(r"(?i)(authorization\s*[:=]\s*bearer\s+)[^\s,;]+|\bbearer\s+[A-Za-z0-9._~+/=-]{12,}").unwrap());
...
mod tests {
    ...
    fn redacts_common_secret_shapes() {
        assert!(!redact("Authorization: Bearer abcdefghijklmnopqrstuvwxyz123456").contains("abcdefghijkl"));
        assert!(!redact(r#"access_token":"abcdefghijklmnopqrstuvwxyz1234567890abcdefghijklmnop"#).contains("abcdefghijklmnopqrstuvwxyz"));
    }
}
